    ...
    public class ArticulosController : Controller
    {
        ...
        public IActionResult Create()
        {
            ArticuloVM artiVM = new ArticuloVM()
            {
                ...
                ListaProveedores = _contenedorTrabajo.Proveedor.GetListaProveedores()
            };
        ...
        public IActionResult Create(ArticuloVM artiVM)
        {
            if (ModelState.IsValid)
            {
                ...
                if (artiVM.Articulo.Id == 0 && archivos.Count() > 0)
                {
                    ...
                    artiVM.Articulo.PrecioCompra = artiVM.Articulo.PrecioCompra;
            ...
            artiVM.ListaProveedores = _contenedorTrabajo.Proveedor.GetListaProveedores();
            return View(artiVM);
        }
        ...
        [HttpPost]
        public IActionResult Comprar([FromBody] CompraVM compraVM)
        {
            try
            {
                var articulo = _contenedorTrabajo.Articulo.Get(compraVM.Id);
                if (articulo == null)
                {
                    return Json(new { success = false, message = "Artículo no encontrado." });
                }

                if (compraVM.Cantidad <= 0)
                {
                    return Json(new { success = false, message = "Cantidad inválida." });
                }

                decimal precioC = compraVM.Precio;

                // Actualizar stock y precio de compra del artículo
                articulo.Stock += compraVM.Cantidad;
                articulo.PrecioCompra = precioC;
                ...
                {
                    ArticuloId = compraVM.Id,
                    Cantidad = compraVM.Cantidad,
                    Precio = compraVM.Precio,
                    FechaCompra = DateTime.Now
                };
                _contenedorTrabajo.Compra.Add(compra);
                _contenedorTrabajo.Save();

                return Json(new { success = true, message = "Compra realizada correctamente." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = $"Error al realizar la compra: {ex.Message}" });
            }
        }
